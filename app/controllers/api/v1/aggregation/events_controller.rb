module Api::V1::Aggregation
  class EventsController < Api::V1::BaseController
    ...
    def index
      ...
      render json: @events, meta: pagination_dict(@events), each_serializer: EventSerializer, adapter: :json
    end

    def show
      render json: @event, serializer: EventSerializer, adapter: :json
    end
    ...
    def create
      ...
      if @event.save && @event.update(commited_at: Time.now)
        render status: :created, json: @event, serializer: EventSerializer, adapter: :json
      else
